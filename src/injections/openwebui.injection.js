(async function (injections) {
    injections.openwebui = async (prompts, ui) => {
        const { origin, module } = ui

        console.log('injected openwebui helper code')
        ...
        async function updatePrompts() {
            ...
            const newPrompts = await Promise.all(scrapedPrompts
                ...
                .map(p => {
                    ...
                    const href = `${origin}/api/v1/chats/${conversationId}`
                    ...
                    return fetch(href, {
                        ...
                        module,
                        title: json.title?.replaceAll('\"', '') || conversationId,
                        href: p.href,
                        string: Object.values(json.chat.messages).filter(v => v.timestamp).sort((a, b) => a.timestamp > b.timestamp ? -1 : 0).pop().content
                    }))
                }))
        ...
        document.addEventListener('keyup', (event) => {
            ...
            const promptTextarea = document.getElementById('chat-textarea')
            ...
            if (event.key === '/') {
                ...
                chrome.runtime.sendMessage({ command: 'search', string: event.target.value }, function (response) {
                    filteredPrompts = response.results.length ? response.results : [...prompts]
                    renderPromptList()
                })
            }
        })
